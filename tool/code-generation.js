...
// Supported languages: Python, PHP, Go`;
    ...
    // Show/hide copy button based on whether actual code is generated
    const copyBtn = document.getElementById('copy-code-btn');
    if (copyBtn) {
        if (hasData && code && !code.includes('// Your generated code will appear here.')) {
    ...
    document.querySelectorAll('#code-generation-languages .btn').forEach(btn => {
...
function initializeCodeGeneration() {
    ...
    document.querySelectorAll('#code-generation-languages .btn').forEach(btn => {
        btn.addEventListener('click', function () {
            const language = this.dataset.codeGenerationLanguage;
            container.dataset.selectedLanguage = language;
            renderCode();
        });
    });
